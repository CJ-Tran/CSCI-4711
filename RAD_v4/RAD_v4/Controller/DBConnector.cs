    ...
    static class DBConnector
    {
        public static void Initialize()
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand("SELECT UName FROM User", conn))
                    {
        ...
        public static User GetUser(string uName, out string storedHash)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("SELECT PwdHash,Type FROM User WHERE UName = @textValue1", conn))
                    {
                    ...
                    //for implementing the hashing algorithm
        ...
        public static KeyList GetKeys()
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("SELECT * from key", conn))
                    {
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                            Key k;
        ...
        public static bool Save(Reservation res)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("UPDATE Key SET Status = @tv1 WHERE Id = @tv2", conn))
                    {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand("SELECT CurrentAssigned FROM Key WHERE Id = @tv1", conn))
                    {
                        ...
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            if (reader.Read())
                                if (!reader.IsDBNull(0))
                                {
                                    ...
                                    using (SQLiteCommand cmd2 = new SQLiteCommand("UPDATE Key SET LastAssigned = @tv1 WHERE Id = @tv2", conn))
                                    {
                                        cmd2.Parameters.AddWithValue("@tv1", nowPrevUser);
                                        cmd2.Parameters.AddWithValue("@tv2", res.KeyID);
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand("UPDATE Key SET CurrentAssigned = @tv1 WHERE Id = @tv2", conn))
                    {
        ...
        public static KeyStatus GetStatus(int key)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("SELECT Status from Key where keyID = @textValue1", conn))
                    {
        ...
        public static void Save(KeyStatus keyStat)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("UPDATE Key SET Status = @textValue1 WHERE Id = @textValue2", conn))
                    {
        ...
        public static void SaveLogin(User user)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    using (SQLiteCommand cmd = new SQLiteCommand("INSERT INTO AccessEvent (User, Time, Type) VALUES (@textValue1, @textValue2, @textValue3)", conn))
                    {
        ...
        private static void AddKeyToDB(int kNum, int roomNum, int status = (int)StatusType.Available, string currentUser = "", string previousUser = "")
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand("INSERT INTO Key(Id,CurrentAssigned,LastAssigned,RoomNum,Status) VALUES (@tv1, @tv2, @tv3, @tv4, @tv5)", conn))
                    {
        ...
        private static void AddUserToDB(string uName, string password, User.AcctType type)
        {
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=KMTS.db;Version=3;"))
            {
                ...
                try
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand("INSERT INTO User(UName, PwdHash, Type) VALUES (@textValue1, @textValue2, @textValue3)", conn))
                    {
                        cmd.Parameters.AddWithValue("@textValue1", uName);
